import { escapeHtml, kFormatter, measureText } from '../utils.js';
...
function getStatItems(stats, hide, showIcons, theme) {
  ...
  if (!hideSet.has('repos')) rows.push({ key: 'repos', value: kFormatter(stats.totalRepos) });
  ...
  return rows.map((row, i) => {
    ...
    const labelWidth = measureText(LABEL_MAP[row.key]);
    const valueX = Math.min(PADDING_LEFT + labelWidth + 20, CARD_WIDTH - PADDING_RIGHT - measureText(row.value) - 10);

    let iconHtml = '';
    if (showIcons) {
      iconHtml = `<svg width="${ICON_SIZE}" height="${ICON_SIZE}" viewBox="0 0 16 16" style="margin-right:4px">${getIconSvg(row.key, theme.icon_color)}</svg>`;
    }

    return `<g transform="translate(0, ${y})">
    <g class="stagger" style="animation-delay: ${delay}ms" transform="translate(${PADDING_LEFT}, 0)">
      <text class="stat bold" y="12.5">${iconHtml}${escapeHtml(LABEL_MAP[row.key])}</text>
...
function getIconSvg(key, color) {
  const fill = `#${color}`;
  switch (key) {
    case 'stars':
      return `<path d="M8 .25a.75.75 0 01.673.418l1.882 3.815 4.21.612a.75.75 0 01.416 1.279l-3.046 2.97.719 4.192a.75.75 0 01-1.088.791L8 12.347l-3.766 1.98a.75.75 0 01-1.088-.79l.72-4.194L.818 6.374a.75.75 0 01.416-1.28l4.21-.611L7.327.668A.75.75 0 018 .25z" fill="${fill}"/>`;
    case 'commits':
      return `<path d="M1.5 8a6.5 6.5 0 1113 0 6.5 6.5 0 01-13 0zm6.5-4.5a4.5 4.5 0 100 9 4.5 4.5 0 000-9z" fill="${fill}"/>`;
    case 'prs':
      return `<path d="M7.177 3.073L9.573.677A.25.25 0 0110 .854v4.792a.25.25 0 01-.427.177L7.177 3.427a.25.25 0 010-.354zM3.75 2.5a.75.75 0 100 1.5.75.75 0 000-1.5zM2.5 3.25a1.25 1.25 0 112.5 0v7.5a1.25 1.25 0 11-2.5 0v-7.5zM3.75 11.5a.75.75 0 100 1.5.75.75 0 000-1.5z" fill="${fill}"/>`;
    case 'issues':
      return `<path d="M8 1.5a6.5 6.5 0 100 13 6.5 6.5 0 000-13zM0 8a8 8 0 1116 0A8 8 0 010 8zm9 3a1 1 0 11-2 0 1 1 0 012 0zm0-5a1 1 0 00-2 0v2a1 1 0 102 0V6z" fill="${fill}"/>`;
    case 'repos':
      return `<path d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 010-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8zM5 12.25v3.25a.25.25 0 00.4.2l1.45-1.087a.25.25 0 01.3 0L8.6 15.7a.25.25 0 00.4-.2v-3.25a.25.25 0 00-.25-.25h-3.5a.25.25 0 00-.25.25z" fill="${fill}"/>`;
    case 'contribs':
      return `<path d="M2.5 1.75a.75.75 0 011.5 0v5.5c0 .69.56 1.25 1.25 1.25h2.5a.75.75 0 010 1.5h-2.5a2.75 2.75 0 01-2.75-2.75v-5.5z" fill="${fill}"/><path d="M6.25 4.75a.75.75 0 011.5 0v2.5a.75.75 0 01-1.5 0v-2.5z" fill="${fill}"/>`;
    default:
      return '';
  }
}
